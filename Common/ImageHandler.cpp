...
void CImageHandler::InitAllImage()
{
	...
	m_avBillBoard[0]  = D3DVERTEX(-0.5f, 0.5f, 0, 0, 0, -1, 0, 0);
	m_avBillBoard[1]  = D3DVERTEX(-0.5f,-0.5f, 0,0, 0, -1, 0, 1);
	m_avBillBoard[2]  = D3DVERTEX(0.5f, 0.5f, 0, 0, 0, -1, 1, 0);
	m_avBillBoard[3]  = D3DVERTEX( 0.5f,-0.5f, 0, 0, 0, -1, 1, 1);
...
HRESULT CImageHandler::DrawBillBoard(LPDIRECT3DDEVICE9 lpDevice, D3DXVECTOR3 vTrans, D3DXVECTOR3 vScale, /*D3DXVECTOR3 vRot, */D3DMATERIAL9 mtrl, /*FLOAT fRotRad, */LPDIRECT3DTEXTURE9  lpddsTextr)
{
	if ( lpDevice )
	{
		D3DMATRIX matTrans;
		...
		//IDirect3DBaseTexture9   AAA;
		lpDevice->SetTexture(0, lpddsTextr);

		IDirect3DVertexBuffer9 *vb9=0;
		lpDevice->CreateVertexBuffer(4*sizeof(D3DVERTEX),0,D3DFVF_VERTEX,D3DPOOL_MANAGED,&vb9,0);

		vb9->Lock(0,0,(void**)m_avBillBoard,0);
		m_avBillBoard[0]  = D3DVERTEX(-0.5f, 0.5f, 0, 0, 0, -1, 0, 0);
		m_avBillBoard[1]  = D3DVERTEX(-0.5f,-0.5f, 0,0, 0, -1, 0, 1);
		m_avBillBoard[2]  = D3DVERTEX(0.5f, 0.5f, 0, 0, 0, -1, 1, 0);
		m_avBillBoard[3]  = D3DVERTEX( 0.5f,-0.5f, 0, 0, 0, -1, 1, 1);
		...
		D3DXMatrixPerspectiveFovLH(&proj,D3DX_PI*0.5f,1366/768,1000.0f,1.0f);
		lpDevice->SetTransform(D3DTS_PROJECTION,&proj);
		if( SUCCEEDED(lpDevice->BeginScene()) )
		{
			lpDevice->SetStreamSource(0,vb9,0,sizeof(D3DVERTEX));
			//D3DFVF_XYZ
			lpDevice->SetFVF(D3DFVF_VERTEX);
			lpDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 4);
