...
BOOL CWHDXGraphicWindow::Create(HINSTANCE hInst, LPTSTR lpCaption, CHAR *pszMenuName, CHAR* pszIconName, BYTE bScreenModeFlag, BYTE bDeviceModeFlag)
{
	...
	HRESULT	hr;
	//2015-06-10
	if ( hr = SUCCEEDED(DirectDrawEnumerateEx(DXGDriverEnumCallbackEx, &hInst,
...
HRESULT CWHDXGraphicWindow::Init3DDeviceObjects()
{
	...
	// ÇöÀç 800*600È­¸é¿¡ ¸Â°Ô 3D ÁÂÇ¥°è¸¦ ¼³Á¤ÇÑ´Ù.
 	D3DXVECTOR3 vEyePt    = D3DXVECTOR3(0, 0, -300.0f);
...
static BOOL gJSEnumDeviceCallBack(const DIDEVICEINSTANCE *inst, void* pParentInfo){
	// ´´½¨DirectInputÉè±¸
	//LPDIRECTINPUT8         g_pDirectInput      = NULL;
	//if(FAILED(DirectInput8Create(NULL,0x0800, IID_IDirectInput8,(void**)&g_pDirectInput, NULL)))
	//	return E_FAIL;
	...
	D3DCAPS9 _devcaps;
	_d3d9->GetDeviceCaps(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,&_devcaps);
	
	
	pDeviceInfo->f3DHardware = _devcaps.Caps&D3DDEVCAPS_HWRASTERIZATION;//pDesc->dwDevCaps & D3DDEVCAPS_HWRASTERIZATION;
	memcpy(&pDeviceInfo->ddDeviceDesc,&_devcaps.Caps, sizeof(D3DCAPS9));

	pDeviceInfo->ddDriverCaps       = pDriverInfo->ddDriverCaps;
	pDeviceInfo->ddHELCaps          = pDriverInfo->ddHELCaps;
	pDeviceInfo->guidDevice         = inst->guidInstance;
	...
	lstrcpyn(pDeviceInfo->szDeviceDesc, inst->tszInstanceName, 255);

	for ( DWORD dwCnt=0; dwCnt<pDriverInfo->dwNumModes; dwCnt++ )
	{
		DDSURFACEDESC2 ddsdMode = pDriverInfo->pddsdModes[dwCnt];
		DWORD dwRenderDepths    = DDBD_32;//pDeviceInfo->ddDeviceDesc.;//  dwDeviceRenderBitDepth;
...
static BOOL WINAPI DXGDriverEnumCallbackEx(GUID *pGUID, LPSTR szDesc, LPSTR szName, LPVOID pContext, HMONITOR hm)
{
    ...
    if ( FAILED(hr = pDD->QueryInterface(IID_IDirect3D9, (void**) &pD3D)) )
    {
		SAFE_RELEASE(pDD);
		MessageBox(NULL, TEXT("[DXGDriverEnumCallbackEx]") TEXT("Can't query IDirect3D9 during enumeration!"), "MirDXG", MB_ICONERROR | MB_OK);
        return D3DENUMRET_OK;
    }
	...
	// ´´½¨DirectInputÉè±¸
	LPDIRECTINPUT8         g_pDirectInput      = NULL;
	if(FAILED(DirectInput8Create((HINSTANCE)pContext,0x0800, IID_IDirectInput8,(void**)&g_pDirectInput, NULL)))
		return E_FAIL;
	//LPDIENUMDEVICESCALLBACK
	hr = g_pDirectInput->EnumDevices(2,(LPDIENUMDEVICESCALLBACK)gJSEnumDeviceCallBack, &stDXGEnumDeviceInfo,DIEDFL_ALLDEVICES);

    //if ( FAILED(hr = pD3D->EnumDevices(DXG3DDeviceEnumCallback, &stDXGEnumDeviceInfo)) )//DIDEVTYPE_MOUSE 2
	if ( FAILED(hr = g_pDirectInput->EnumDevices(2,(LPDIENUMDEVICESCALLBACK)gJSEnumDeviceCallBack, &stDXGEnumDeviceInfo,DIEDFL_ALLDEVICES)))//hr = g_pDirectInput->EnumDevices(2,DXG3DDeviceEnumCallback, &stDXGEnumDeviceInfo,DIEDFL_ALLDEVICES)) )
	{
