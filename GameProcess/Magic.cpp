		                 ...
		                 SHORT shTargetTileX, SHORT shTargetTileY, CActor* pxOwner, INT nTarget)
{
	...
	if ( pstMagicSpr )
	{
 	...
 	//	D3DXVECTOR3 vNorm(0, 0, -1);
		//m_avMagic[0]  = D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0), vNorm, 0, 0);
		//m_avMagic[1]  = D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0), vNorm, 0, 1);
		//m_avMagic[2]  = D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0), vNorm, 1, 0);
		//m_avMagic[3]  = D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0), vNorm, 1, 1);

		m_avMagic[0]  = D3DVERTEX(-0.5f,0.5f,0,0,0,-1,0,0);//D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0), vNorm, 0, 0);
		m_avMagic[1]  = D3DVERTEX(-0.5f,-0.5f,0,0,0,-1,0,1);//D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0), vNorm, 0, 1);
		m_avMagic[2]  = D3DVERTEX(0.5f,0.5f,0,0,0,-1,1,0);//D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0), vNorm, 1, 0);
		m_avMagic[3]  = D3DVERTEX(0.5f,-0.5f,0,0,0,-1,1,1);//D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0), vNorm, 1, 1);
		//------------------------------------------------------------------------------------------------------------
...
// 임시.
	if ( !m_bFixed )
	{
					...
					{
		g_xGameProc.m_xFlyingTail.SetFlyTailParticle(D3DXVECTOR3(m_shScrnX+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPX+ptArr[m_bDir16].x, m_shScrnY+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPY+ptArr[m_bDir16].y, 0));
		g_xGameProc.m_xSmoke.SetSmokeParticle(D3DXVECTOR3(m_shScrnX+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPX+ptArr[m_bDir16].x, m_shScrnY+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPY+ptArr[m_bDir16].y, 0));
					}
					else if ( m_bMagicNum == _SKILL_FIREBALL )
					{
		g_xGameProc.m_xFlyingTail.SetFlyTailParticle(D3DXVECTOR3(m_shScrnX+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPX+ptArr2[m_bDir16].x, m_shScrnY+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPY+ptArr2[m_bDir16].y, 0));
		g_xGameProc.m_xSmoke.SetSmokeParticle(D3DXVECTOR3(m_shScrnX+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPX+ptArr2[m_bDir16].x, m_shScrnY+m_pxMagicImg->m_lpstNewCurrWilImageInfo->shPY+ptArr2[m_bDir16].y, 0));
					}
	}
	...
	//				D3DUtil_SetRotationMatrix(matRot, D3DXVECTOR3(1.0f,1.0f,0.0f), g_PI/m_dwCurrFrame);
	...
	*/				mtrl.Diffuse.a = (m_bOpacity+1)/255.0f;
					...
					vertices[0]=m_avMagic[0];
					vertices[1]=m_avMagic[1];
					vertices[2]=m_avMagic[2];
					vertices[3]=m_avMagic[3];
					vb->Unlock();
...
//		   if ( 마법이 통과할수 없으면 )
	...
	if ( bCrash )
	{
		...
		//D3DXVECTOR3 vNorm(0, 0, -1);
		//(float x=0,float y=0,float z=0,float nx=0,float ny=0,float nz=0,float u=0,float v=0)
		m_avMagic[0]  = D3DVERTEX(-0.5f,0.5f,0,0,0,-1,0,0);//D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0), vNorm, 0, 0);
		m_avMagic[1]  = D3DVERTEX(-0.5f,-0.5f,0,0,0,-1,0,1);//D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0), vNorm, 0, 1);
		m_avMagic[2]  = D3DVERTEX(0.5f,0.5f,0,0,0,-1,1,0);//D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0), vNorm, 1, 0);
		m_avMagic[3]  = D3DVERTEX(0.5f,-0.5f,0,0,0,-1,1,1);//D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0), vNorm, 1, 1);
