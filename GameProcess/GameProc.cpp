...
void CGameProcess::Init()
{
 ...
 //	D3DXVECTOR3 vNorm(0, 0, -1);
	//m_avBillboard[0]  = D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0), vNorm, 0, 0);
	//m_avBillboard[1]  = D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0), vNorm, 0, 1);
	//m_avBillboard[2]  = D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0), vNorm, 1, 0);
	//m_avBillboard[3]  = D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0), vNorm, 1, 1);


	m_avBillboard[0]  = D3DVERTEX(-0.5f,0.5f,0,0,0,-1,0,0);//D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0), vNorm, 0, 0);
	m_avBillboard[1]  = D3DVERTEX(-0.5f,-0.5f,0,0,0,-1,0,1);//D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0), vNorm, 0, 1);
	m_avBillboard[2]  = D3DVERTEX(0.5f,0.5f,0,0,0,-1,1,0);// D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0), vNorm, 1, 0);
	m_avBillboard[3]  = D3DVERTEX(0.5f,-0.5f,0,0,0,-1,1,1);//D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0), vNorm, 1, 1);
			...
			if ( pstItem->shTileX == m_ptCurrTile.x && pstItem->shTileY == m_ptCurrTile.y )
			{
				...
				if ( g_xMainWnd.Get3DDevice() )
				{
					if( SUCCEEDED(g_xMainWnd.Get3DDevice()->BeginScene()) )
					{
						...
						g_xMainWnd.Get3DDevice()->SetStreamSource(0,&m_avBillboard,0,sizeof(D3DVERTEX));
...
*/						int nCenterX = (pstItem->shTileX-m_xMap.m_shStartViewTileX)*_CELL_WIDTH +_VIEW_CELL_X_START-m_xMap.m_shViewOffsetX+_CELL_WIDTH /2;
						...
						vertices[0]=m_avBillboard[0];
						vertices[1]=m_avBillboard[1];
						vertices[2]=m_avBillboard[2];
						vertices[3]=m_avBillboard[3];
						vb->Unlock();
						...
						if ( m_xMap.m_pxTileImg[nObjFileIdx]->NewSetIndex(nImgIdx) )
						{
  								 ...
  								 m_xMap.m_pxTileImg[nObjFileIdx]->m_lpstNewCurrWilImageInfo->shHeight == 32 )
							{
								...
								else
								{
									...
									D3DMATERIAL9 mtrl;
									D3DXVECTOR3	vecTrans(nStartX, nStartY, 0);
									D3DXVECTOR3	vecScale(m_xMap.m_pxTileImg[nObjFileIdx]->m_lpstNewCurrWilImageInfo->shWidth, m_xMap.m_pxTileImg[nObjFileIdx]->m_lpstNewCurrWilImageInfo->shHeight, 1);
									D3DXVECTOR3 a;
									D3DUtil_InitMaterial(mtrl, (FLOAT)255/255.0f, (FLOAT)255/255.0f, (FLOAT)255/255.0f);
...
LRESULT CGameProcess::OnKeyDown(WPARAM wParam, LPARAM lParam)
{
	...
	D3DXVECTOR3 vTrans  = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 vRotate = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 vScale  = D3DXVECTOR3(1.0f, 1.0f, 1.0f);

	g_xMainWnd.Get3DDevice()->GetTransform(D3DTS_VIEW, &matView);
	...
	{
	case 'L':
		g_xMainWnd.Get3DDevice()->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
		break;
	case 'M':
		g_xMainWnd.Get3DDevice()->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);
		break;
	case 'N':
		{
			SetNextProc();
	...
	case VK_LEFT:
		{
			...
			g_xMainWnd.Get3DDevice()->SetTransform(D3DTS_VIEW, &matTemp);
		}
		break;
	case VK_RIGHT:
		{
