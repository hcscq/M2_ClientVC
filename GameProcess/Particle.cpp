...
void CParticleSystem::InitSystem()
{
	...
	m_fAirFiction	 = -0.05f;
	m_vecBounding[0] = D3DXVECTOR3(0, 0, 0);
	m_vecBounding[1] = D3DXVECTOR3(0, 0, 0);
	m_vecEnvironment = D3DXVECTOR3(0, 300, 0);


	D3DXVECTOR3 vNorm(0, 0, -1);
	m_avShape[0] =  D3DVERTEX(-0.5f,-0.5f,0,0,0,-1,0.0f,1.0f);//D3DVERTEX(D3DXVECTOR3(-0.5f,-0.5f, 0.0f ), vNorm, 0.0f, 1.0f);
	m_avShape[1] =  D3DVERTEX(0.5f,-0.5f,0,0,0,-1,0.0f,0.0f);//D3DVERTEX(D3DXVECTOR3( 0.5f,-0.5f, 0.0f ), vNorm, 0.0f, 0.0f);
	m_avShape[2] =  D3DVERTEX(-0.5f,0.5f,0,0,0,-1,1.0f,1.0f);//D3DVERTEX(D3DXVECTOR3(-0.5f, 0.5f, 0.0f ), vNorm, 1.0f, 1.0f);
	m_avShape[3] =  D3DVERTEX(0.5f,0.5f,0,0,0,-1,1.0f,0.0f);//D3DVERTEX(D3DXVECTOR3( 0.5f, 0.5f, 0.0f ), vNorm, 1.0f, 0.0f);
}
...
void CParticleSystem::UpdateCrash(WORD wNum)
{
	if ( !m_pxParticle[wNum].m_bIsDead )
	{
		...
		if ( m_pxParticle[wNum].m_vecPos.y <= m_vecBounding[0].y || m_pxParticle[wNum].m_vecPos.y >= m_vecBounding[1].y )
		{
			...
			FLOAT fBefore = m_pxParticle[wNum].m_wDelay * (m_vecBounding[1].y - fOldY)/(m_pxParticle[wNum].m_vecPos.y - fOldY);							// Ãæµ¹Àü.
			FLOAT fAfter  = m_pxParticle[wNum].m_wDelay * (m_pxParticle[wNum].m_vecPos.y - m_vecBounding[1].y)/(m_pxParticle[wNum].m_vecPos.y - fOldY);	// Ãæµ¹ÈÄ.
...
void CSnow::SetupSystem(WORD wCnt, FLOAT fWidth, FLOAT fGround)
{
	...
	SetEnvFactor(-0.05f, D3DXVECTOR3(10, 100, 0));
}


void CSnow::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	INT nGenCnt = 0; 
...
//				SetEnvFactor(fRand, D3DXVECTOR3(-fRand, fRand*100, 0));
...
HRESULT CSnow::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	if ( lpDevice )
	{
		if( SUCCEEDED(lpDevice->BeginScene()) )
		{
			...
			lpDevice->GetTransform(D3DTS_WORLD, &matWorldOriginal);
...
//					SetBlendRenderState(lpDevice, _BLEND_NORMAL, mtrl);
					...
					//lpDevice->SetRenderState(D3DRS_COLORKEYENABLE, TRUE);
					...
					g_xMainWnd.Get3DDevice()->CreateVertexBuffer(4*sizeof(D3DVERTEX),0,D3DFVF_VERTEX,D3DPOOL_MANAGED,&vb,0);
					D3DVERTEX *vertices;
					vb->Lock(0,0,(void **)&vertices,0);
					vertices[0]=m_avShape[0];
					vertices[1]=m_avShape[1];
					vertices[2]=m_avShape[2];
					vertices[3]=m_avShape[3];
					vb->Unlock();

					g_xMainWnd.Get3DDevice()->SetStreamSource(0,vb,0,sizeof(D3DVERTEX));
					g_xMainWnd.Get3DDevice()->SetFVF(D3DFVF_VERTEX);
					g_xMainWnd.Get3DDevice()->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 4);
...
//			ResetBlendenderState(lpDevice);
			...
			//lpDevice->SetRenderState(D3DRS_COLORKEYENABLE, FALSE);
			lpDevice->SetRenderState(D3DRS_SRCBLEND , D3DBLEND_ONE);
...
void CSnow::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
    m_pxParticle[wNum].m_vecPos		= D3DXVECTOR3((FLOAT)GetRandomNum(0, m_fWidth)/*(-400, 1200)*/, (FLOAT)GetRandomNum(-300,  0), 0.0f);
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-30,  30), (FLOAT)GetRandomNum(70, 100), 0.0f);
	m_pxParticle[wNum].m_vecAccel	= D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_pxParticle[wNum].m_vecOldPos	= D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_pxParticle[wNum].m_vecLocalForce = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
void CRain::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	INT nGenCnt = 0; 
	INT nSpeedRate = (nLoopTime / 17);
	if ( nSpeedRate < 1 ) 		
...
HRESULT CRain::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	if ( lpDevice )
	{
		if( SUCCEEDED(lpDevice->BeginScene()) )
...
void CRain::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
    ...
    m_pxParticle[wNum].m_vecPos		= D3DXVECTOR3((FLOAT)GetRandomNum(-100, m_fWidth+100)/*(-400, 1200)*/, (FLOAT)GetRandomNum(-500, 0), 0.0f);
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)/*GetRandomNum(0,  10)*/0, 500/*(FLOAT)GetRandomNum(10, 30)*/, 0.0f);
//	m_pxParticle[wNum].m_vecAccel	= D3DXVECTOR3(0.0f, 0.0f, 0.0f);
//	m_pxParticle[wNum].m_vecOldPos	= D3DXVECTOR3(0.0f, 0.0f, 0.0f);
//	m_pxParticle[wNum].m_vecLocalForce = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
void CFly::SetupSystem(WORD wCnt)
{
	...
	SetEnvFactor(-0.05f, D3DXVECTOR3(100, 1000, 0));
}


void CFly::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	BYTE bRate;
	INT nGenCnt = 0; 
...
HRESULT CFly::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	if ( lpDevice )
	{
		if( SUCCEEDED(lpDevice->BeginScene()) )
...
void CFly::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
	// m_bRainIntensity ---> ºñÀÇ °­µµ°¡ ½ÉÇØÁú¼ö·Ï gencnt, mass, size, delay, velocityµî¿¡ ¿µÇâÀ» ¹ÌÄ£´Ù.

	ZeroMemory(&m_pxParticle[wNum], sizeof(CParticle));
    m_pxParticle[wNum].m_vecPos		= vecGenPos;
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-30, 30), (FLOAT)GetRandomNum(-20, 40), 0.0f);
...
void CBoom::SetupSystem(WORD wCnt)
{
	...
	SetEnvFactor(-0.05f, D3DXVECTOR3(0, 200, 0));
}


void CBoom::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	INT nSpeedRate = (nLoopTime / 17);
	INT nPartCnt = 0;
...
HRESULT CBoom::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	INT nPartCnt = 0;
	if ( lpDevice )
	{
...
void CBoom::SetBoomParticle(D3DXVECTOR3 vecGenPos)
{
	INT nGenCnt = 0;
	for ( INT nCnt = 0; nCnt < m_wNum; nCnt++ )
	{
...
void CBoom::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
	ZeroMemory(&m_pxParticle[wNum], sizeof(CParticle));
    m_pxParticle[wNum].m_vecPos		= vecGenPos;
//    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-40, 40), (FLOAT)GetRandomNum(-100, -20), 0.0f);
//    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-60, 60), (FLOAT)GetRandomNum(-80, 0), 0.0f);
//    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-50, 50), (FLOAT)GetRandomNum(-100, -20), 0.0f);
//    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-60, 60), (FLOAT)GetRandomNum(-150, -50), 0.0f);
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-75, 75), (FLOAT)GetRandomNum(-180, -50), 0.0f);
...
void CFlyingTail::SetFlyTailParticle(D3DXVECTOR3 vecGenPos)
{
	INT nGenCnt = 0;
	for ( INT nCnt = 0; nCnt < m_wNum; nCnt++ )
	{
...
void CFlyingTail::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	BYTE bRate;
...
HRESULT CFlyingTail::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	INT  nPartCnt;
...
void CFlyingTail::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
	ZeroMemory(&m_pxParticle[wNum], sizeof(CParticle));
    m_pxParticle[wNum].m_vecPos		= vecGenPos;
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-50, 50), (FLOAT)GetRandomNum(-30, 60), 0.0f);
...
void CSmoke::SetupSystem(WORD wCnt)
{
	...
	SetEnvFactor(-0.05f, D3DXVECTOR3(0, 1000, 0));
}


void CSmoke::UpdateSystem(INT nLoopTime, D3DXVECTOR3 vecGenPos)
{
	BYTE bRate;
	INT  nSpeedRate, nPartCnt;
	D3DXVECTOR3 vecAddVel;
...
//				vecAddVel = D3DXVECTOR3((FLOAT)GetRandomNum(-1, 1), GetRandomNum(-2, 0), 0.0f);
				...
				vecAddVel = D3DXVECTOR3((FLOAT)GetRandomNum(-1, 1), 0.0f, 0.0f);
...
void CSmoke::SetSmokeParticle(D3DXVECTOR3 vecGenPos)
{
	INT nGenCnt = 0;
	for ( INT nCnt = 0; nCnt < m_wNum; nCnt++ )
	{
...
HRESULT CSmoke::RenderSystem(LPDIRECT3DDEVICE9 lpDevice)
{
	INT  nPartCnt;
...
void CSmoke::SetParticleDefault(WORD wNum, D3DXVECTOR3 vecGenPos)
{
	ZeroMemory(&m_pxParticle[wNum], sizeof(CParticle));
//	vecGenPos.x += (FLOAT)GetRandomNum(-5, 5);
//	vecGenPos.y += (FLOAT)GetRandomNum(-8, 2);
    m_pxParticle[wNum].m_vecPos		= vecGenPos;
    m_pxParticle[wNum].m_vecVel		= D3DXVECTOR3((FLOAT)GetRandomNum(-8, 8), /*(FLOAT)GetRandomNum(-15, -10)*/0.0f, 0.0f);
//	m_pxParticle[wNum].m_wLife		= (FLOAT)GetRandomNum(500, 1000);
	m_pxParticle[wNum].m_wLife		= (FLOAT)GetRandomNum(150, 400);
	m_pxParticle[wNum].m_fMass		= 1000.0f;
...
void CElec::SetupElec()
{
	...
	for ( INT nCntY = 0; nCntY < (_MAX_ELEC_NODEY+1); nCntY++ )
	{
		for ( INT nCntX = 0; nCntX < (_MAX_ELEC_NODEX+1); nCntX++ )
		{
			...
			m_avElec[nCntY][nCntX] = D3DVERTEX(fXPos, fYPos, 0, 0,0,-1, fu, fv);
//			m_avElec[nCntY][nCntX] = D3DLVERTEX(D3DXVECTOR3(fXPos, fYPos, 0), RGB(0, 255, 0), RGB(255, 0, 0), fu, fv);
		}
	}
...
//		FLOAT fRandX = _RAND_FLOAT*1.3f	 - 1.0f;
		...
		for ( INT nCntX = 0; nCntX < (_MAX_ELEC_NODEX+1); nCntX++ )
		{
			m_avElec[nCntY][nCntX]._x = m_avSaveElec[nCntY][nCntX]._x+fRandX;
...
/*
	...
	sprintf(szBuff, "->¶óµð¾È:[%f] %f", m_fRadian, fDiv);
	g_xMainWnd.PutsHan(g_xMainWnd.GetBackBuffer(), 0, 100, RGB(250, 125, 0), RGB(0, 0, 0), szBuff);
*/
}

HRESULT CElec::RenderElec(LPDIRECT3DDEVICE9 lpDevice)
{
	if ( lpDevice )
	{
		if( SUCCEEDED(lpDevice->BeginScene()) )
...
//			lpDevice->SetRenderState(D3DRS_LIGHTING, FALSE);
			...
			lpDevice->CreateIndexBuffer(_ELEC_INDICES*sizeof(WORD),D3DUSAGE_WRITEONLY,D3DFMT_INDEX32,D3DPOOL_MANAGED,&IB,0);
			WORD *indices=0;
			IB->Lock(0,0,(void**)&indices,0);
			for(int i=0;i<_ELEC_INDICES;i++)
				indices[i]=m_pwElecIndices[i];
			IB->Unlock();

			IDirect3DVertexBuffer9 *VB=0;
			lpDevice->CreateVertexBuffer(_ELEC_VERTICES*sizeof(D3DVERTEX),D3DUSAGE_WRITEONLY,D3DFVF_LVERTEX,D3DPOOL_MANAGED,&VB,0);
			D3DVERTEX *vertex=0;
			VB->Lock(0,0,(void**)&vertex,0);
			for(int i=0;i<_ELEC_VERTICES;i++)
				vertex[i]=m_avElec[i];
			VB->Unlock();

			lpDevice->SetStreamSource(0,VB,0,sizeof(D3DVERTEX));
			lpDevice->SetIndices(IB);
			lpDevice->SetFVF(D3DFVF_LVERTEX);

			//Í¼ÔªÊý ÉÐ²»È·¶¨
			lpDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,0,_ELEC_VERTICES,0,_ELEC_INDICES/3);
