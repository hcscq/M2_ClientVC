...
CMist::CMist()
{
    ...
    m_avBillboard[0] = D3DVERTEX(-400.0f, 300.0f, 0,0,0,-1,0,0);//D3DVERTEX(D3DXVECTOR3(-400.0f, 300.0f, 0), vNorm, 0, 0);
    m_avBillboard[1] = D3DVERTEX(-400.0f, -300.0f, 0,0,0,-1,0,1);//D3DVERTEX(D3DXVECTOR3(-400.0f,-300.0f, 0), vNorm, 0, 1);
    m_avBillboard[2] = D3DVERTEX(400.0f, 300.0f, 0,0,0,-1,1,0);//D3DVERTEX(D3DXVECTOR3( 400.0f, 300.0f, 0), vNorm, 1, 0);
    m_avBillboard[3] = D3DVERTEX(400.0f, -300.0f, 0,0,0,-1,1,1);//D3DVERTEX(D3DXVECTOR3( 400.0f,-300.0f, 0), vNorm, 1, 1);
...
HRESULT CMist::DrawMist()
{
	if ( g_xMainWnd.Get3DDevice() )
	{
		...
		if( SUCCEEDED(g_xMainWnd.Get3DDevice()->BeginScene()) )
		{
			...
			g_xMainWnd.Get3DDevice()->CreateVertexBuffer(4*sizeof(D3DVERTEX),D3DUSAGE_WRITEONLY,D3DFVF_VERTEX,D3DPOOL_MANAGED,&VB,0);
			D3DVERTEX *vertex=0;
			VB->Lock(0,0,(void**)&vertex,0);
			vertex[0]=m_avBillboard[0];
			vertex[1]=m_avBillboard[1];
			vertex[2]=m_avBillboard[2];
			vertex[3]=m_avBillboard[3];
			VB->Unlock();
...
{
 	D3DXVECTOR3 vNorm(0, 0, -1);
	m_avBillboard[0] = D3DVERTEX( D3DXVECTOR3(-0.5f,-5.0f, 0.0f ), vNorm, 0.0f, 1.0f );
    m_avBillboard[1] = D3DVERTEX( D3DXVECTOR3( 0.5f,-5.0f, 0.0f ), vNorm, 0.0f, 0.0f );
    m_avBillboard[2] = D3DVERTEX( D3DXVECTOR3(-0.5f, 5.0f, 0.0f ), vNorm, 1.0f, 1.0f );
    m_avBillboard[3] = D3DVERTEX( D3DXVECTOR3( 0.5f, 5.0f, 0.0f ), vNorm, 1.0f, 0.0f );
}
...
{
 	D3DXVECTOR3 vNorm(0, 0, -1);
	m_avBillboard[0] = D3DVERTEX( D3DXVECTOR3(-2.0f,-2.0f, 0.0f ), vNorm, 0.0f, 1.0f );
    m_avBillboard[1] = D3DVERTEX( D3DXVECTOR3(-2.0f, 2.0f, 0.0f ), vNorm, 0.0f, 0.0f );
    m_avBillboard[2] = D3DVERTEX( D3DXVECTOR3( 2.0f,-2.0f, 0.0f ), vNorm, 1.0f, 1.0f );
    m_avBillboard[3] = D3DVERTEX( D3DXVECTOR3( 2.0f, 2.0f, 0.0f ), vNorm, 1.0f, 0.0f );
}
