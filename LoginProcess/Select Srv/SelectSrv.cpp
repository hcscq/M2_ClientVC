	...
	if ( g_xMainWnd.Get3DDevice() )
	{
		...
		if( SUCCEEDED(g_xMainWnd.Get3DDevice()->BeginScene()))
		{
			...
			ZeroMemory(&matView, sizeof(D3DMATRIX));
		 	D3DXVECTOR3 vEyePt    = D3DXVECTOR3(0, 0, -(float)((g_xMainWnd.m_rcWindow.bottom - g_xMainWnd.m_rcWindow.top)/2));
			...
			g_xMainWnd.Get3DDevice()->SetTransform(D3DTS_VIEW,       &matView);
			g_xMainWnd.Get3DDevice()->SetTransform(D3DTS_PROJECTION, &matProj);
			...
			//g_xMainWnd.Get3DDevice()->SetRenderState( D3DRS_COLORKEYENABLE, TRUE);//D3DRENDERSTATE_COLORKEYENABLE
